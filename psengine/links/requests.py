...
from ..common_models import RFBaseModel, IdNameType
...
class LinksFilterObjects(RFBaseModel):
    ...
    technical: Optional[FilterTechnical] = Field(
        ...
        examples=[FilterTechnical(
            timeframe="-30d",
            events=["TTPAnalysis"],
            connected_entities=["idn:google.com"]
        )]
    )
...
class LinksLimitsObjects(RFBaseModel):
    ...
    search_scope: Optional[Literal["small", "medium", "large"]] = Field(
        ...
        alias='search_scope',
        description=
        'The Links API searches for links in references, which is a performance intensive search. To ensure a fast response and a balance between different sources among events, there are some filters and limits applied.'
    ...
    def validate_per_entity_type(cls, v: Optional[int]) -> Optional[int]:
        ...
        if v is None:
            return v
        if 0 > v or v > 2147483647:
            raise ValueError("per_entity_type must be int (greater than "
...
class LinksSearchIn(RFBaseModel):
    ...
    entities: list[str] = Field(
        alias='entities',
        description='Entities for which to search for links. Uses Recorded Future '
                    'entity IDs.',
        examples=['QCwdoU']
    )
    ...
    limits: Optional[LinksLimitsObjects] = Field(
        ...
        examples=[LinksLimitsObjects(
            search_scope="small",
            per_entity_type=1000
        )]
    )