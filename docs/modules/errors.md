...
This section focuses on error handling. We will not cover every module, but since every module is designed in a very similar way, the same concepts apply.
In the following examples, we show how to understand errors generated by PSEngine and how to interpret or handle them.

The first few examples show the errors as they would appear without proper error handling; then we show how to handle them.
...
In this example, we set the `RF_TOKEN` environment variable to an empty string before importing `psengine`. This is just a workaround to make the example reproducible, to show what happens if you run PSEngine without the variable configured.
