...
2. All the methods mentioned on point 1 accept a `field` parameter to increase or reduce the information retrived for each alert. The following fields are always requested: `id`, `log`, `title`, `rule` no matter which field you specify. 
    - `search` uses only the required fields by default if the `fields` parameter is not speficified.
...
This example starts with the assumption that you have a list of alert IDs retrived by a search, or a colleague or another integration/security tool. We will use only two alert IDs for demonstration.
...
This example starts with the assumption that you have an alert ID retrived by a search, or a colleague or another integration/security tool. We will use only two alert IDs for demonstration.
