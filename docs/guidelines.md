...
We follow the [https://semver.org](https://semver.org) standard, and while we try to avoid breaking changes with new minor or patch releases, it is always advised to pin a specific version of PSEngine in your `pyproject.toml`. For example:
...
When a field, feature, or endpoint is deprecated, we will add a deprecation warning with an alternative if available. Three minor releases later, it will be removed.
...
The Minimum Calls column indicates the minimum number of entities (IOCs, alerts, etc.) after which you should start getting a decent time gain with multithreading.
...
The code structure below is how we are structuring the code for integrations. This is more of an internal preference that we have been using; you are free to follow it if you like.
